...
allNewsEl.classList.add("allNews");
body.appendChild(allNewsEl);

news.forEach((el) => createMarking(el));

function createMarking({ title, headerBgSrc, category, body, date }) {
  const articleEl = createEl("article");
  articleEl.classList.add("news");

  const heartEl = createEl("button");
  heartEl.classList.add("likeNews", "colorGrey");

  const faHeartEl = createEl("i");
  faHeartEl.classList.add("fa-solid", "fa-heart");

  const titleDivEl = createEl("div");
  titleDivEl.classList.add("titleDiv");

  const imgEl = createEl("img");
  imgEl.classList.add("imgNews");

  const titleEl = createEl("h2");
  titleEl.classList.add("titleNews");

  const bodyNewsEl = createEl("div");
  bodyNewsEl.classList.add("bodyNews");

  const categoryNewsEl = createEl("h3");
  categoryNewsEl.classList.add("categoryNews", "colorGrey");

  const textNewsEl = createEl("p");
  textNewsEl.classList.add("textNews");

  const dateNewsEl = createEl("p");
  dateNewsEl.classList.add("dateNews", "colorGrey");

  appendChildToEl(articleEl, [
    appendChildToEl(heartEl, [faHeartEl]),
    appendChildToEl(titleDivEl, [imgEl, titleEl]),
    appendChildToEl(bodyNewsEl, [categoryNewsEl, textNewsEl, dateNewsEl]),
  ]);
  ...
  function appendChildToEl(el, addList) {
    addList.forEach((appEl) => {
  ...
  categoryNewsEl.textContent = category;
  textNewsEl.textContent = body;
  dateNewsEl.textContent = date;
  imgEl.src = headerBgSrc;
  imgEl.alt = title;

  allNewsEl.appendChild(articleEl);

  heartEl.addEventListener("click", heartColorCenge);
  function heartColorCenge() {
    faHeartEl.classList.toggle("redHeart");
  }
}
